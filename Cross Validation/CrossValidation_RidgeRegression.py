...
for size in dataset_sizes:
    ...
    mae_cv = cross_val_score(ridge_model, subset_X, subset_y, cv=5, scoring=mae_scorer)
    r2_cv  = cross_val_score(ridge_model, subset_X, subset_y, cv=5, scoring=r2_scorer)

    mae_results.append(np.mean(mae_cv))
    r2_results.append(np.mean(r2_cv))

    print(f"[Ridge][frac={size:.1f}]  "
...
plt.title("Ridge Regression – MAE and R^2 vs. Dataset Size")
plt.show()
